...
def classify_basic_move(
    ...
    # Lost -> drawable rescue: never worse than Good
    if before_state == "Lost" and after_state in ("Equalish", "Winning", "Won"):
        if player_delta >= 300:
            if LABEL_RANK[label] > LABEL_RANK["Good"]:
                label = "Good"

    # Already totally winning → soften
    if before_state == "Won" and after_state == "Won":
        label = soften_label(label, "Mistake")
        if label == "Mistake" and cpl <= 250:
            label = "Inaccuracy"
...
def detect_miss(
    *,
    ...
    # ------------------------------------------------------------------
    # 1) SPECIAL CASE: missed free material
    # ------------------------------------------------------------------
...
# class GreatMoveParams:
#     """
#     Tunables for 'Great' (!) moves:

#     - Big improvement for the mover based on delta_eval (WHITE POV).
#     - No CP loss vs engine best from the PRE position.
#     """
#     min_improvement_cp: int = 600   # >= 10 pawns improvement for the mover
#     max_cp_loss_cp: int = 0          # CP lost must be <= this (0 = no loss)
...
# def detect_great_move(
#     *,
#     eval_before_white: float,                 # eval BEFORE move, White POV
#     eval_after_white: float,                  # eval AFTER move, White POV
#     eval_best_pre_white: Optional[float],     # best move eval from PRE, White POV
#     eval_played_pre_white: Optional[float],   # played move eval from PRE, White POV
#     mover_color: str,                         # 'w' or 'b'
#     params: Optional[GreatMoveParams] = None,
# ) -> GreatMoveResult:
#     """
#     Your 'Great' definition:

#       1) Work in WHITE POV:
#             delta_eval = eval_after_white - eval_before_white

#          For White move:
#             mover_improvement = delta_eval
#          For Black move:
#             mover_improvement = -delta_eval  (because more negative is good for Black)

#       2) Condition:
#             mover_improvement >= min_improvement_cp  (e.g. 1000 cp)

#       3) CP loss vs best from PRE:

#          For White:
#             cp_loss = best_eval_pre - played_eval_pre

#          For Black:
#             cp_loss = played_eval_pre - best_eval_pre

#          Require:
#             cp_loss <= max_cp_loss_cp   (0 → no loss, negative = we did even better)
#     """
#     if params is None:
#         params = GreatMoveParams()

#     # Need both best and played PRE evals for proper CP loss computation
#     if eval_best_pre_white is None or eval_played_pre_white is None:
#         return GreatMoveResult(
#             is_great=False,
#             reason="missing_best_or_played_pre_eval",
...
#     # 1) delta_eval in WHITE POV
#     delta_eval_white = eval_after_white - eval_before_white

#     if mover_color == "w":
#         mover_improvement = delta_eval_white
#     else:
#         # For Black, a *more negative* eval is good.
#         mover_improvement = -delta_eval_white

#     # 2) require big improvement for the mover
#     if mover_improvement < params.min_improvement_cp:
#         return GreatMoveResult(
#             is_great=False,
#             reason="improvement_too_small",
#             mover_improvement_cp=mover_improvement,
#             cp_loss_for_mover_cp=None,
#             delta_eval_white_cp=delta_eval_white,
#         )

#     # 3) CP loss vs best (from mover POV)
#     if mover_color == "w":
#         cp_loss_for_mover = eval_best_pre_white - eval_played_pre_white
#     else:
#         cp_loss_for_mover = eval_played_pre_white - eval_best_pre_white
